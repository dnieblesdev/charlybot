...
export async function execute(interaction: ChatInputCommandInteraction) {
  try {
    ...
    let game = await RouletteService.getActiveGame(guildId, channelId);
    let isNewGame = false;
    ...
    const bets = await RouletteService.getGameBets(guildId, gameId);
...
async function spinRoulette(gameId: number, interaction: CommandInteraction) {
  try {
    ...
    if (bets.length === 0) {
      ...
      await RouletteService.cancelGame(guildId, gameId, interaction.guild!);
      return;
    }
    ...
    // Girar la ruleta
    const game = await RouletteService.spin(guildId, gameId);
