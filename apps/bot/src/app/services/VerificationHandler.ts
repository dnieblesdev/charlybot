...
export async function handleVerificationModalSubmit(
  ...
  try {
    ...
    const classes = await getAllClasses(interaction.guild.id);
...
export async function handleClassSelect(
  ...
  try {
    ...
    const classConfig = await getClass(interaction.guild.id, selectedClass);
...
export async function handleSubclassSelect(
  ...
  try {
    ...
    const classConfig = await getClass(interaction.guild.id, className);
    if (!classConfig) {
    ...
    await createVerificationRequest(interaction.guild.id, request);
