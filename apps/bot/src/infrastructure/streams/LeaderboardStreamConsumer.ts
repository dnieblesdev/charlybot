  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        logger.error('Leaderboard consumer: failed to process event', {
          entryId: entry.id,
          retryCount,
        ...
        if (retryCount >= LEADERBOARD_STREAM_CONFIG.MAX_RETRIES) {
          // Move to DLQ
          await valkey.streamAdd(keys.dlq, {
            ...entry.fields,
    ...
    if (isRunning) {
      logger.error('Leaderboard consumer: loop error', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
...
export async function startLeaderboardStreamConsumer(): Promise<void> {
  ...
  if (!valkey.isConnected()) {
    logger.warn('LeaderboardStreamConsumer: Valkey not connected, deferring start');
    return;
  }

  isRunning = true;
  logger.info('LeaderboardStreamConsumer started', { consumerId, stream: keys.stream });
