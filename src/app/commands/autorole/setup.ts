    ...
    if (messageId) {
      try {
        ...
        const existingConfig =
          await AutoRoleRepo.getAutoRoleByMessageId(messageId);
        ...
        if (existingConfig) {
          ...
          setupSessions.set(sessionId, {
            ...
            mappings: existingConfig.mappings.map((m) => ({
              roleId: m.roleId,
 ...
 */
function startCollector(interaction: any, sessionId: string) {
 ...
 */
async function handleEditMapping(interaction: any, sessionId: string) {
  const session = setupSessions.get(sessionId);
 ...
 */
async function handleRemoveMapping(interaction: any, sessionId: string) {
  const session = setupSessions.get(sessionId);
  if (!session || session.mappings.length === 0) {
    await interaction.reply({
 ...
 */
async function handleCustomizeEmbed(interaction: any, sessionId: string) {
  try {
 ...
 */
async function handleToggleMode(interaction: any, sessionId: string) {
  const session = setupSessions.get(sessionId);
  if (!session) {
    await interaction.reply({
 ...
 */
async function handleFinish(interaction: any, sessionId: string) {
  const session = setupSessions.get(sessionId);
  if (!session) {
    await interaction.reply({
      content: "❌ Sesión no encontrada.",
  ...
  try {
    ...
    if (existingAutoRole) {
      ...
      await AutoRoleRepo.updateAutoRole(existingAutoRole.id, {
        mode: session.mode,
        embedTitle: session.embedTitle,
        embedDesc: session.embedDesc,
        embedColor: session.embedColor,
      ...
      await AutoRoleRepo.removeAllRoleMappings(existingAutoRole.id);
      autoRoleId = existingAutoRole.id;
    } else {
      ...
      const autoRole = await AutoRoleRepo.createAutoRole({
    ...
    for (const mapping of session.mappings) {
      await AutoRoleRepo.addRoleMapping({
